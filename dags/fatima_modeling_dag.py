...
with DAG(
    ...
    transformations = {
        "view_fact_orders": f"""
            ...
            FROM
                `ready-de-25.landing.fatima_orders` o
            LEFT JOIN 
                `ready-de-25.landing.fatima_order_review` r on o.order_id = r.order_id
            LEFT JOIN 
                `ready-de-25.landing.fatima_order_payments` p on o.order_id = p.order_id  
        """,
        ...
        "view_dim_customer": f"""
            ...
            FROM 
            `ready-de-25.landing.fatima_customers`
        """,
        ...
        FROM 
            ready-de-25.landing.fatima_products` p
        LEFT JOIN 
        `ready-de-25.landing.fatima_product_category_name_translation` e on  p.product_category_name = e.product_category_name
        LEFT JOIN
         `ready-de-25.landing.fatima_order_items` oi on p.product_id = oi.product_id
        """,
        ...
        "view_dim_order_items": f"""
           ...
           SELECT DISTINCT order_id, order_item, product_id, seller_id, price
            FROM 
         `ready-de-25.landing.fatima_order_items`
        """,
    }
